...
const ViewAsset = () => {
    ...
    const fetchAsset = async () => {
      try {
        ...
        if (res.data.assetLocation) {
          try {
            const locationRes = await axios.get(`https://backend-ps-tali.onrender.com/locations/${res.data.assetLocation}`);
            setLocationName(locationRes.data.assetLocation || 'Unknown Location');
          } catch {
  ...
  const handleDownload = () => {
    ...
    asset.stockLocations?.forEach((loc, index) => {
      doc.text(`${index + 1}. ${loc.location || loc.storeName} - ${loc.quantity}`, 12, 105 + index * 10);
    });
            ...
            <div className='flex justify-between items-start'>
              <div className='flex gap-4'>
                <img
                  ...
                  alt="Asset"
                  className='w-60 h-40 object-cover rounded-md border border-gray-200'
                />
                <div>
                  <h2 className='text-xl font-semibold'>{asset?.assetName}</h2>
                  <p className='text-sm text-gray-500'>{locationName}</p>
                  <p className='text-xs mt-2 text-gray-600'>VIN: {asset?.assetId || 'N/A'}</p>
                  <button className='mt-3 bg-gray-800 text-white text-sm px-4 py-1 rounded'>Assign</button>
                  <div className='mt-3'>
                    <label className='text-gray-600 text-sm'>Justification</label>
                    <textarea
                      rows={3}
                      className='w-full border mt-1 p-2 rounded-md text-sm border-gray-300'
              ...
              <div className='flex gap-2'>
                ...
                <button onClick={handleDownload} className='border px-3 py-1 rounded text-sm text-gray-600 border-gray-300'>Download</button>
              </div>
            </div>

            <div className='mt-6 grid grid-cols-2 gap-6'>
              <div>
                <h3 className='text-md font-semibold text-gray-700 mb-2'>Overview</h3>
                <ul className='text-sm text-gray-600 space-y-1'>
                  <li><strong>Make:</strong> {asset?.make || 'N/A'}</li>
                  <li><strong>Model:</strong> {asset?.model || 'N/A'}</li>
                  <li><strong>Year:</strong> {asset?.year || 'N/A'}</li>
                  <li><strong>Condition:</strong> {asset?.condition || 'N/A'}</li>
                  <li><strong>Body Type:</strong> {asset?.bodyType || 'N/A'}</li>
                  <li><strong>Color:</strong> {asset?.color || 'N/A'}</li>
                </ul>
              </div>
              <div>
                <h3 className='text-md font-semibold text-gray-700 mb-2'>Features</h3>
                <ul className='text-sm text-gray-600 space-y-1'>
                  <li><strong>Mileage:</strong> {asset?.mileage || 'N/A'}</li>
                  <li><strong>Drivetrain:</strong> {asset?.drivetrain || 'N/A'}</li>
                  <li><strong>Doors:</strong> {asset?.doors || 'N/A'}</li>
                  <li><strong>Max Seating:</strong> {asset?.maxSeating || 'N/A'}</li>
                </ul>
              </div>
            </div>

            <div className='mt-8'>
              <h3 className='text-md font-semibold text-gray-700 mb-2'>Inspection Details</h3>
              <p className='text-sm text-gray-600'>Inspected by: Kofi Baah</p>
              <p className='text-sm text-gray-600'>Approved by: Jordan Owusu</p>
              <p className='text-sm text-gray-600'>Date Uploaded: 18 June, 2025</p>
            </div>

            <div className='mt-8'>
              <h3 className='text-md font-semibold text-gray-700 mb-2'>Similar Assets</h3>
              <div className='flex gap-4 text-sm text-gray-600'>
                <div>
                  <img src='/car-placeholder.png' alt='Asset' className='w-28 h-16 rounded-md object-cover'/>
