  ...
  useEffect(() => {
    const fetchAsset = async () => {
      try {
        ...
        // Fetch location name using the assetLocation field (which contains the location ID)
        if (res.data.assetLocation) {
          try {
            ...
            // Use the assetLocation field from the location object as the display name
    ...
    <div className='flex flex-row'>
      ...
      <div className='w-[80vw]'>
        ...
        <div className='bg-[#f0f1f3] min-h-[90%] p-4'>
          <div className='bg-white rounded-md shadow-sm border border-white p-4'>
            <div className='flex flex-row justify-between '>
              <p className='font-semibold mb-4'>Asset: {asset?.assetName || 'Unnamed Asset'}</p>
              <div className='flex flex-row gap-3 text-[13px]'>
                <div className='flex flex-row border-[0.5px] px-3 gap-1 rounded-sm border-gray-300 text-gray-600'>
                  <p className='mt-3'><FiEdit2 /></p>
                  <button>Edit</button>
            ...
            <div className='flex flex-row gap-[40%]'>
              <div>
                <div className=''>
                  ...
                  <div className='space-y-5 mt-3'>
                    ...
                    <div className='flex flex-row gap-32'>
                      <p className='text-gray-400 text-[16px] font-semibold'>Asset Location</p>
                      <p className='text-gray-600 font-medium'>{locationName || 'Loading...'}</p>
                    </div>
                    <div className='flex flex-row gap-[42%]'>
                      <p className='text-gray-400 text-[16px] font-semibold'>Status</p>
                      <p className={`font-medium ${asset?.status === 'Available' ? 'text-green-600' : 'text-red-600'}`}>
              ...
              <div className='mt-7'>
                ...
                <div className='mt-4 space-y-5'>
                  <div className='flex flex-row gap-3'>
                    <p className='text-green-600'>Available:</p>
                    <p className='text-gray-600'>{asset?.unit || 0}</p>
                  </div>
                  <div className='flex flex-row gap-3'>
                    <p className='text-red-500'>Status:</p>
                    <p className={`${asset?.status === 'Available' ? 'text-green-600' : 'text-red-600'}`}>
                      {asset?.status || 'Unknown'}
                    </p>
                  </div>
                </div>
            ...
            <div>
              ...
              <div className='flex flex-col space-y-5 mt-4'>
                {asset?.stockLocations && asset.stockLocations.length > 0 ? (
                  asset.stockLocations.map((loc, idx) => (
                    <div key={idx} className='flex flex-row border-b-[0.5px] border-gray-300 justify-between w-[60%]'>
                      <p className='ml-2 text-gray-400 text-[16px] font-semibold'>{loc.location || loc.storeName || 'Unknown Location'}</p>
                      <p className='mr-5 text-gray-600'>{loc.quantity}</p>
                    </div>
                  ))
                ) : (
                  <div className='flex flex-row justify-center w-[60%] py-4'>
                    <p className='text-gray-400 text-sm'>No stock locations available</p>
                  </div>
                )}
              </div>
            </div>
